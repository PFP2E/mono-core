  ...
  return (
    <header className='sticky top-0 left-0 z-50 w-full backdrop-blur-md'>
      <nav className='bg-background/50 border-border sm-px-12 flex w-full flex-wrap items-center justify-between border-b px-4 py-2.5 md:px-28'>
        ...
        <Link
          href='/'
          className='pointer-events-auto relative z-10 flex cursor-pointer items-center gap-2 transition-opacity hover:opacity-80'
        >
          <Image
            src='/logo.png'
            alt='PFP2E Logo'
          ...
          />
          <span className='text-primary pointer-events-auto text-2xl font-bold'>
            PFP2E
          </span>
        </Link>

        {/* Search Bar Section */}
        <div className='bg-card/80 border-border relative order-3 my-2 flex h-11 w-full max-w-sm flex-grow items-center gap-4 rounded-full border px-4 py-2.5 sm:order-2 sm:my-0 sm:flex-grow-0'>
          ...
          {showResults && filteredCampaigns.length > 0 && (
            <div className='bg-card border-border absolute top-full right-0 left-0 z-50 mt-2 max-h-96 overflow-y-auto rounded-lg border shadow-lg'>
              {filteredCampaigns.map(campaign => (
                <Link
                  href={`/campaign/${campaign.campaignName}`}
                  key={campaign.id}
                >
                  <Card className='hover:bg-muted/50 cursor-pointer rounded-none border-0'>
          ...
          {showResults && searchQuery && filteredCampaigns.length === 0 && (
            <div className='bg-card border-border absolute top-full right-0 left-0 z-50 mt-2 rounded-lg border p-4 shadow-lg'>
