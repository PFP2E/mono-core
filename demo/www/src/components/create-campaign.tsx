...
export function CreateCampaign() {
  ...
  const [collectionInfo, setCollectionInfo] = React.useState({ name: '', found: false })
  const [fetchingCollection, setFetchingCollection] = React.useState(false)
  ...
  const isStep4Valid = () => {
    ...
    if (createNewToken) {
      ...
      return rewardTokenDropdown || (tokenInfo.symbol && tokenInfo.symbol !== 'Error' && tokenInfo.symbol !== 'Unknown')
    }
  }
  ...
  const isStep5Valid = () => {
    ...
    if (impressionsEnabled && (!impressionsCount.trim() || !impressionsDays.trim())) return false
  ...
  const getDurationInputClass = () => {
    ...
    return durationDays.trim() ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
  }

  const getFollowerCountInputClass = () => {
    if (!followerCountEnabled || !eligibilityEnabled) return ''
    return followerCount.trim() ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
  }

  const getImpressionsCountInputClass = () => {
    if (!impressionsEnabled) return ''
    return impressionsCount.trim() ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
  }

  const getImpressionsDaysInputClass = () => {
    if (!impressionsEnabled) return ''
    return impressionsDays.trim() ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500'
  }
  ...
  const handleTokenAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value && value.startsWith('0x')) {
      ...
      const knownToken = mockRewardTokens.find(t => t.address.toLowerCase() === value.toLowerCase())
      if (knownToken) {
  ...
  const fetchNftInfo = async (collectionAddress: string) => {
    ...
    try {
      ...
      const nftContract = new ethers.Contract(collectionAddress, nftABI, provider)
  ...
  const handleCollectionAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value && value.startsWith('0x')) {
      ...
      const knownCollection = mockCollections.find(c => c.address.toLowerCase() === value.toLowerCase())
      if (knownCollection) {
                ...
                <>
                  <div>
                    ...
                    <Select
                      ...
                      value={nftInfo.name && nftInfo.name !== 'Error' ? '' : nftCollection}
                      disabled={nftInfo.name && nftInfo.name !== 'Error'}
                    >
                      <SelectTrigger className={nftInfo.name && nftInfo.name !== 'Error' ? 'opacity-50 cursor-not-allowed' : ''}>
                  ...
                  <div>
                    ...
                    {nftCollection && nftCollection.startsWith('0x') && !fetchingNft && (
                  ...
                  <div className='space-y-6'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-muted-foreground mb-2'>
                        Upload an image of the object, brand, or style you want to use as reference
                      </p>
      ...
      case 4:
        return (
          <>
            <div className='space-y-4'>
              <div>
                ...
                <Select
                  ...
                  disabled={createNewToken || (tokenInfo.symbol && tokenInfo.symbol !== 'Error' && tokenInfo.symbol !== 'Unknown')}
                >
                  <SelectTrigger className={cn(
                    (createNewToken || (tokenInfo.symbol && tokenInfo.symbol !== 'Error' && tokenInfo.symbol !== 'Unknown')) ? 'opacity-50 cursor-not-allowed' : ''
                  )}>
              ...
              <div>
                ...
                <Input
                  ...
                  className={createNewToken ? 'opacity-50 cursor-not-allowed' : ''}
                />
                {fetchingToken && (
                ...
                {createNewToken && (
                  <div className='space-y-3 mt-3'>
                    <h3 className='text-lg font-semibold text-primary'>Create New Reward Token</h3>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                      ...
                      <div>
                        <Label htmlFor='poolPercentage'>% to Reward Pool *</Label>
                        <Input
                          id='poolPercentage'
            ...
            <div className='mt-3'>
              <Label htmlFor='dailyRewardPercentage'>Daily Reward Pool Emit %</Label>
              <Select
      ...
      case 5:
        return (
          <>
            <div className='space-y-4'>
              <div>
                <Label className='text-base font-semibold mb-4'>PFP2E Verification Rules</Label>
                <p className='text-sm text-muted-foreground mb-6'>
                  Choose from the curated core conditions to ensure quality participation in your campaign.
                </p>
                ...
                <div className='space-y-4 mb-6'>
                  ...
                  <div className='p-3 border rounded-lg bg-muted/50'>
                    <div>
                      ...
                      <p className='text-xs text-muted-foreground'>Must wear campaign PFP (NFT or Derivative) to earn rewards</p>
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-4 mb-6'>
                  <div className='flex items-center space-x-2'>
                        ...
                        />
                    <h3 className='text-lg font-semibold'>DURATION (Consistency of Support)</h3>
                  </div>
                  <p className='text-sm text-muted-foreground'>
                  ...
                  <div className='space-y-3'>
                    <div className='flex items-center justify-between p-3 border rounded-lg'>
                      <div>
                        <Label className='font-medium'>Minimum consecutive days staked</Label>
                        <p className='text-xs text-muted-foreground'>Must keep PFP unchanged for X days in a row</p>
                      </div>
                      <div>
                        <Input 
                ...
                <div className='space-y-4 mb-6'>
                  <div className='flex items-center space-x-2'>
                    ...
                    />
                    <h3 className='text-lg font-semibold'>ELIGIBILITY (User Qualifiers)</h3>
                  </div>
                  <p className='text-sm text-muted-foreground'>
                  ...
                  <div className='space-y-3'>
                    <div className='flex items-center justify-between p-3 border rounded-lg'>
                      <div>
                        <Label className='font-medium'>Minimum follower count</Label>
                        <p className='text-xs text-muted-foreground'>Basic reach filter</p>
                      </div>
                      <div className='flex items-center space-x-3'>
                        <Input 
                          className={`w-20 ${getFollowerCountInputClass()}`}
                          placeholder='e.g. 1000' 
                          disabled={!followerCountEnabled || !eligibilityEnabled} 
                          value={followerCount}
                    ...
                    <div className='flex items-center justify-between p-3 border rounded-lg'>
                      <div>
                        <Label className='font-medium'>Must be Twitter Blue</Label>
                        <p className='text-xs text-muted-foreground'>Filter for paying/serious users</p>
                      </div>
                      <div className='flex items-center space-x-3'>
                        <Select disabled={!twitterBlueEnabled || !eligibilityEnabled} value={twitterBlueValue} onValueChange={setTwitterBlueValue}>
                ...
                <div className='space-y-4 mb-6'>
                  <div className='flex items-center space-x-2'>
                        ...
                        />
                    <h3 className='text-lg font-semibold'>PERFORMANCE (Output / Earned Visibility)</h3>
                  </div>
                  <p className='text-sm text-muted-foreground'>
                    Reward users based on what they generate, not just who they are.
                  </p>
                  <div className='space-y-3'>
                    <div className='flex flex-col md:flex-row md:items-center md:justify-between p-3 border rounded-lg space-y-3 md:space-y-0'>
                      <div>
                        <Label className='font-medium'>Impressions in last X days</Label>
                        <p className='text-xs text-muted-foreground'>Great signal of real visibility</p>
                      </div>
                ...
                <div className='space-y-4'>
                  <div className='flex items-center space-x-2'>
                        ...
                        />
                    <h3 className='text-lg font-semibold'>CONNECTION / BEHAVIOR HISTORY</h3>
                  </div>
                  <p className='text-sm text-muted-foreground'>
                  ...
                  <div className='space-y-3'>
                    <div className='flex flex-col md:flex-row md:items-center md:justify-between p-3 border rounded-lg space-y-3 md:space-y-0'>
                      <div>
                        <Label className='font-medium'>Must have PFP verified on multiple platforms</Label>
                        <p className='text-xs text-muted-foreground'>Cross-platform loyalty</p>
                      </div>
                      <div className='flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2'>
                        ...
                        <div className='flex items-center space-x-2'>
                          <Checkbox 
                            checked={platformIG} 
                            onCheckedChange={(checked) => setPlatformIG(checked as boolean)}
                            disabled={!platformsEnabled}
                          />
                          <Label className='text-xs'>IG</Label>
                          <Checkbox 
                            checked={platformFB} 
                            onCheckedChange={(checked) => setPlatformFB(checked as boolean)}
                            disabled={!platformsEnabled}
                          />
                          <Label className='text-xs'>FB</Label>
                          <Checkbox 
                            checked={platformTT} 
                            onCheckedChange={(checked) => setPlatformTT(checked as boolean)}
                            disabled={!platformsEnabled}
                          />
                        ...
                        <div className='flex items-center space-x-2'>
                          <Checkbox 
                            ...
                            onCheckedChange={(checked) => setPlatformFarcaster(checked as boolean)}
                            disabled={!platformsEnabled}
                          />
                          <Label className='text-xs'>Farcaster</Label>
                          <Checkbox 
                            checked={platformDiscord} 
                            onCheckedChange={(checked) => setPlatformDiscord(checked as boolean)}
                            disabled={!platformsEnabled}
                          />
            ...
            <div className='space-y-4'>
              <div className='text-muted-foreground space-y-3 rounded-lg border bg-zinc-50 p-6 dark:bg-zinc-900'>
                <dl className='grid grid-cols-2 gap-x-4 gap-y-2 text-sm'>
                  ...
                  <dt className='text-foreground font-medium'>Campaign Description</dt>
                  ...
                  {campaignType === 'nft' && (
                    <>
                      <dt className='text-foreground font-medium'>NFT Collection Name</dt>
                      <dd>
                        {nftInfo.name && nftInfo.name !== 'Error' ? nftInfo.name : getCollectionName(nftCollection)}
                      </dd>
                      <dt className='text-foreground font-medium'>NFT Contract Address</dt>
                  ...
                  <dt className='text-foreground font-medium'>Reward Token Name</dt>
                  <dd>
                    {createNewToken ? tokenName : (rewardTokenDropdown ? mockRewardTokens.find(t => t.address === rewardTokenDropdown)?.name : tokenInfo.symbol)}
                  </dd>

                  <dt className='text-foreground font-medium'>Reward Token Address</dt>
                  <dd className='truncate font-mono text-xs'>
                    {createNewToken ? 'Created At Launch' : (rewardToken || 'N/A')}
                  </dd>

                  {createNewToken && (
                    <>
                      <dt className='text-foreground font-medium'>New Token Total Supply</dt>
                      <dd>{initialSupply || 'N/A'}</dd>
                      <dt className='text-foreground font-medium'>New Token To Reward Pool</dt>
                      <dd>{poolPercentage ? `${poolPercentage}%` : 'N/A'}</dd>
                    </>
                  )}

                  <dt className='text-foreground font-medium'>Daily Emit Rate %</dt>
                  <dd>{dailyRewardPercentage ? `${dailyRewardPercentage}%` : 'N/A'}</dd>
                </dl>
              </div>

              {/* Conditions Section */}
              <div className='text-muted-foreground space-y-3 rounded-lg border bg-zinc-50 p-6 dark:bg-zinc-900'>
                <h3 className='text-foreground font-medium mb-4'>Conditions</h3>
                <dl className='grid grid-cols-2 gap-x-4 gap-y-2 text-sm'>
                                  {/* Always show Wear Campaign PFP */}
                <dt className='text-foreground font-medium'>Wear Campaign PFP</dt>
                  <dd>Yes</dd>

                  {/* Show enabled conditions with their values */}
                  {durationEnabled && (
                    <>
                      <dt className='text-foreground font-medium'>Minimum consecutive days</dt>
                  ...
                  {followerCountEnabled && (
                    <>
                      <dt className='text-foreground font-medium'>Minimum follower count</dt>
                  ...
                  {twitterBlueEnabled && (
                    <>
                      <dt className='text-foreground font-medium'>Must be Twitter Blue</dt>
                  ...
                  {impressionsEnabled && (
                    <>
                      <dt className='text-foreground font-medium'>Impressions in last X days</dt>
                      <dd>{impressionsCount || 'N/A'} impressions in {impressionsDays || 'N/A'} days</dd>
                    </>
                  )}

                  {platformsEnabled && (
                    <>
                      <dt className='text-foreground font-medium'>PFP verified on platforms</dt>
                      <dd>
                        {[
                          platformIG && 'IG',
                          platformFB && 'FB', 
